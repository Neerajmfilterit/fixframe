...
const AlertDialogContent = ({ children }: any) => <div className="w-full">{children}</div>;
const AlertDialogHeader = ({ children }: any) => <div className="mb-4">{children}</div>;
const AlertDialogTitle = ({ children }: any) => <h2 className="text-lg font-semibold mb-2 text-gray-900">{children}</h2>;
const AlertDialogDescription = ({ children }: any) => <p className="text-gray-600 mb-4 text-sm">{children}</p>;
...
export default function EnhancedChartCustomizer({ 
  ...
  // Create Plain HTML/CSS Bundle
  const createHtmlCssBundle = () => {
...
<html lang="en">
  <head>
    ...
    <meta name="description" content="${cleanProjectName} dashboard" />
  ...
  <body>
    ...
    <main class="canvas">
      <!-- Your dashboard layout exported as static blocks -->
    </main>

    <footer class="footer">Generated by Fixframe</footer>
  </body>
</html>`;

    const css = `*{box-sizing:border-box}body{margin:0;font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;background:#f5f7fb;color:#111827}
.dashboard-header{background:#111827;color:#e5e7eb;padding:16px 24px}
.header-title{margin:0;font-size:20px;font-weight:600}
.canvas{min-height:calc(100vh - 96px);padding:24px}
...
.footer{padding:12px 24px;color:#6b7280;font-size:12px}`;
  ...
  const generateZipFile = React.useCallback(async () => {
    try {
      ...
      const projectFiles = createHtmlCssBundle();
  ...
  React.useEffect(() => {
    console.log('EnhancedChartCustomizer: parentShowCodeModal changed:', parentShowCodeModal);
  ...
  const [selectedFramework, setSelectedFramework] = useState<'react-recharts' | 'react-chartjs' | 'vue' | 'angular' | 'python'>('react-recharts');
  const [selectedStyling, setSelectedStyling] = useState<'tailwind' | 'css' | 'bootstrap'>('tailwind');
  const [includeDarkMode, setIncludeDarkMode] = useState(true);
  const [exportStructure, setExportStructure] = useState<'single' | 'multiple'>('single');
  ...
  const handleApiFetch = async () => {
    ...
    try {
      ...
      setApiError(error instanceof Error ? error.message : 'Couldn\'t fetch data. Please check URL or payload.');
    } finally {
      setIsLoadingApi(false);
    }
  };
...
import { 
  ...
  XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer 
    ...
    const chartComponents = charts.map((chart, index) => {
      const componentName = `${chart.title?.replace(/\s+/g, '') || `Chart${index + 1}`}`;
      const data = chart.data?.length > 0 ? chart.data : [
        { name: 'Jan', value: 400 },
        { name: 'Feb', value: 300 },
        { name: 'Mar', value: 200 }
      ];
  ...
  <div className="chart-container">
    ...
    const dataDefinitions = charts.map((chart, index) => {
      const dataName = `${chart.title?.replace(/\s+/g, '') || `chart${index + 1}`}Data`;
      const data = chart.data?.length > 0 ? chart.data : [
        { name: 'Jan', value: 400 },
        { name: 'Feb', value: 300 },
        { name: 'Mar', value: 200 }
...
${charts.map((chart, index) => {
  ...
  return `chart${index + 1}_data = pd.DataFrame(${JSON.stringify(data, null, 2)})`;
}).join('\n')}
...
${charts.map((chart, index) => {
  ...
  switch (chart.type) {
    case 'bar':
      return `fig${index + 1} = px.bar(${dataName}, x='name', y='value', title='${chart.title || `Chart ${index + 1}`}')`;
    case 'line':
      return `fig${index + 1} = px.line(${dataName}, x='name', y='value', title='${chart.title || `Chart ${index + 1}`}')`;
    case 'area':
      return `fig${index + 1} = px.area(${dataName}, x='name', y='value', title='${chart.title || `Chart ${index + 1}`}')`;
    case 'donut':
      return `fig${index + 1} = px.pie(${dataName}, values='value', names='name', title='${chart.title || `Chart ${index + 1}`}', hole=0.4)`;
    default:
      return `fig${index + 1} = px.bar(${dataName}, x='name', y='value', title='${chart.title || `Chart ${index + 1}`}')`;
  }
}).join('\n')}
...
# Access: http://localhost:8050`;
  ...
  const generateAISuggestions = () => {
    ...
    if (charts.length === 0) {
      ...
      if (charts.length > 3) {
        suggestions.push("Consider adding responsive breakpoints for better mobile experience");
      }
      ...
      if (selectedFramework === 'react-recharts') {
        suggestions.push("Add state management (Redux/Zustand) for complex data handling");
      }
      if (includeDarkMode) {
      ...
      suggestions.push("Implement data refresh functionality with real-time updates");
    }
  ...
  const exportAsFile = () => {
    ...
    const filename = `${projectName.replace(/\s+/g, '_')}_dashboard.${extension}`;
  ...
  const exportCurrentData = () => {
    if (!selectedChart || !Array.isArray(selectedChart.data) || selectedChart.data.length === 0) {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${selectedChart?.title?.replace(/[^a-z0-9]/gi, '_').toLowerCase() || 'chart'}_data.csv`;
    a.click();
    window.URL.revokeObjectURL(url);
  };
  ...
  return (
    <div className={`w-80 h-screen ${bgClass} border-l ${borderClass} flex flex-col`}>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide">
        <div className={`${activeTab === 'comments' ? 'h-full overflow-hidden' : 'h-full p-4'}`}>
        {!selectedChart ? (
          <div className="flex flex-col items-center justify-center h-full text-center p-8">
            <Code className="w-16 h-16 text-gray-400 mb-4" />
            <h3 className={`text-lg font-semibold ${textClass} mb-2`}>No Component Selected</h3>
            <p className={`text-sm ${textSecondaryClass} mb-6`}>
              Click on a component in the canvas to customize it, or use the Code button to export all components.
            </p>
            <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-blue-900/20 border border-blue-700/50' : 'bg-blue-50 border border-blue-200'}`}>
              <p className={`text-sm ${isDarkMode ? 'text-blue-300' : 'text-blue-700'}`}>
                <strong>💡 Tip:</strong> The Code button in the toolbar will generate code for all components on the canvas.
              </p>
            </div>
          </div>
        ) : activeTab === 'size' && (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className={`text-sm ${textSecondaryClass}`}>Fetch data from REST API endpoints</p>
                      </div>
                    </div>
                    <ChevronDown className={`w-5 h-5 transition-transform ${showApiConnection ? 'rotate-180' : ''}`} />
                  </button>
                </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <p className={`text-xs ${textSecondaryClass} mt-1`}>
                          Enter your authorization token (e.g., Bearer token, Basic auth, API key)
                        </p>
                      </div>
                      ...
                      {['POST', 'PUT', 'PATCH'].includes(httpMethod) && (
                        <div>
                          ...
                          />
                          <p className={`text-xs ${textSecondaryClass} mt-1`}>
                            Enter JSON data to send with your {httpMethod} request
                          </p>
                        </div>
                      )}
                      ...
                      <div className="space-y-2">
                        {apiAuth.trim() && (
                          <div className={`p-2 rounded-lg border-l-4 ${isDarkMode ? 'bg-green-900/10 border-green-500 text-green-300' : 'bg-green-50 border-green-400 text-green-800'}`}>
                            <div className="flex items-center gap-2">
                              <Check className="w-3 h-3" />
                              <span className="text-xs font-medium">Authorization will be included in request</span>
                            </div>
                          </div>
                        )}
                        <button
                        ...
                        <div className={`p-3 rounded-lg border-l-4 ${isDarkMode ? 'bg-green-900/10 border-green-500 text-green-300' : 'bg-green-50 border-green-400 text-green-800'}`}>
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                <h6 className="font-medium text-sm">Response Preview</h6>
                                <p className="text-xs opacity-90">Data fetched successfully!</p>
                              </div>
                            </div>
                            <button
                            ...
                            >
                              {showResponsePreview ? <EyeOff className="w-3 h-3" /> : <Eye className="w-3 h-3" />}
                            </button>
                          </div>
                          {showResponsePreview && (
                            <div className={`mt-3 p-2 rounded text-xs font-mono overflow-auto max-h-32 ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
                ...
                <div className="absolute inset-0 bg-white bg-opacity-90 dark:bg-gray-800 dark:bg-opacity-90 flex items-center justify-center z-10">
                  <div className="text-center">
                    ...
                    <p className={`mt-2 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                      {isModalOpening ? 'Opening...' : 'Generating project files...'}
                    </p>
                  </div>
                </div>
              )}
              <div className="space-y-4">
                {/* Project Structure Preview (HTML/CSS only) */}
      ...
      {showFullscreenPreview && (
        <div className="fixed inset-0 bg-black bg-opacity-90 backdrop-blur-sm flex items-center justify-center z-[60]">
          <div className={`relative w-full h-full max-w-7xl max-h-[95vh] rounded-xl shadow-2xl ${
            isDarkMode ? 'bg-gray-800' : 'bg-white'
          }`}>
            {/* Fullscreen Header */}
            <div className={`flex items-center justify-between p-4 border-b ${
              ...
              <div className="flex items-center gap-3">
                ...
                <span className={`text-lg font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                  {selectedFramework === 'python' ? 'Dashboard.py' : 'Dashboard.js'} - Full Preview
                </span>
              </div>
            ...
            <div className={`h-[calc(100%-80px)] overflow-auto ${
              ...
              <pre className={`p-8 text-sm font-mono leading-relaxed ${
      ...
      <AlertDialog open={showDownloadDialog} onOpenChange={setShowDownloadDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Download HTML/CSS Bundle</AlertDialogTitle>
            <AlertDialogDescription>
              This will download a simple HTML and CSS folder you can open directly in a browser.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
