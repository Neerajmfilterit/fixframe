...
export async function POST(request: NextRequest) {
  try {
    const { emails, projectName, shareLink, shareLinks, senderName, projectId } = await request.json();
    ...
    const emailPromises = emails.map(async (user) => {
      try {
        ...
        const userLink = shareLinks?.find((link: any) => link.email === user.email)?.link || shareLink;
        ...
        return { email: user.email, success: false, error: error instanceof Error ? error.message : 'Unknown error' };
      }
    });
    ...
    return NextResponse.json({
      ...
      projectId: projectId,
      redirectUrl: projectId ? `/builder?project=${projectId}` : '/builder'
    });
